				  ...
				  ei_font_t font, ei_color_t color, const ei_rect_t *clipper)
{
	...
	if (text != NULL)
	{
		...
		dest.top_left.y = (clipper == NULL) ? where->y : (clipper->top_left.y > source.top_left.y + where->y) ? clipper->top_left.y
																											  : where->y;
		dest.size.width = (clipper == NULL) ? hw_surface_get_size(surface_source).width : (hw_surface_get_size(surface_source).width > clipper->top_left.x + clipper->size.width - where->x) ? (clipper->top_left.x + clipper->size.width - where->x < 0) ? 0 : clipper->top_left.x + clipper->size.width - where->x
																																															 : hw_surface_get_size(surface_source).width;
		dest.size.height = (clipper == NULL) ? hw_surface_get_size(surface_source).height : (hw_surface_get_size(surface_source).height > clipper->top_left.y + clipper->size.height - where->y) ? (clipper->top_left.y + clipper->size.height - where->y < 0) ? 0 : clipper->top_left.y + clipper->size.height - where->y
